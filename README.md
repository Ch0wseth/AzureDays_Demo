# 🤖 GitHub Copilot CLI — Atelier (Lab) Orange Boosted

> **Atelier hands-on** pour découvrir **GitHub Copilot CLI** (`@github/copilot`) dans un terminal — version Azure Days.
> Vous suivez ce guide à votre rythme sur votre poste : pas d'IDE requis, juste `copilot` et ce dépôt.
> Comptez ~60–90 minutes pour dérouler les 8 exercices. Chaque exercice est indépendant : vous pouvez en sauter.
...
4. [Suivi des tokens](#-suivi-des-tokens-tout-au-long-de-latelier)
5. [EXERCICE 1 — Génération de code](#-exercice-1--génération-de-code)
6. [EXERCICE 2 — Génération de tests](#-exercice-2--génération-de-tests)
7. [EXERCICE 3 — Génération de documentation](#-exercice-3--génération-de-documentation)
8. [EXERCICE 4 — Modes Standard / Plan / Autopilot](#-exercice-4--modes-standard--plan--autopilot)
9. [EXERCICE 5 — Customisations : `AGENTS.md`, Instructions, Skills, Agents & MCP](#-exercice-5--customisations--agentsmd-instructions-skills-agents--mcp)
10. [EXERCICE 6 — Gestion du contexte](#-exercice-6--gestion-du-contexte)
11. [EXERCICE 7 — Bonnes pratiques de prompting](#-exercice-7--bonnes-pratiques-de-prompting)
12. [EXERCICE 8 — Optimisation des tokens (Caveman Mode)](#-exercice-8--optimisation-des-tokens-caveman-mode)
13. [Structure du projet](#-structure-du-projet)
14. [Checklist avant de commencer](#-checklist-avant-de-commencer)
15. [FAQ / Troubleshooting](#-faq--troubleshooting)
16. [Reproduire l'atelier — Guide express](#-reproduire-latelier--guide-express)

---

## 📋 Pré-requis

### Outils à installer AVANT l'atelier
...
> ⚠️ Le CLI `copilot` **ne lit pas** `.github/mcp.json` (qui sert à VS Code). Sa config MCP est utilisateur : `~/.copilot/mcp-config.json`. Sans cette étape, `playwright` n'apparaît pas dans `/mcp` et l'EXERCICE 5e échoue.
...
Pour **playwright** (requis pour l'EXERCICE 5e) :
...
> 💡 **Astuce shell avec `!`** : depuis l'invite `copilot`, tout ce qui commence par `!` est exécuté tel quel dans votre shell (PowerShell / bash), et la sortie reste visible dans le contexte de la session. Quelques exemples utiles pendant l'atelier :
>
> ```text
> !npm test
> !git status
...
## 📊 Suivi des Tokens (tout au long de l'atelier)
...
**Activer la télémétrie locale (recommandé pour l'atelier)** :
...
À la fin de l'atelier, filtrer les tokens :
...
### Tableau de suivi — À garder ouvert pendant l'atelier
...
## 🎬 EXERCICE 1 — Génération de Code

### 🎯 Objectif
Vérifier que Copilot CLI génère un fichier JS complet à partir d'une consigne courte qui s'appuie sur un fichier existant.
...
## 🎬 EXERCICE 2 — Génération de Tests

### 🎯 Objectif
Vérifier que Copilot CLI génère une suite Jest complète via un **skill** custom.
...
## 🎬 EXERCICE 3 — Génération de Documentation

### 🎯 Objectif
Vérifier que Copilot CLI génère du JSDoc enrichi avec exemples.
...
## 🎬 EXERCICE 4 — Modes Standard / Plan / Autopilot
...
## 🎬 EXERCICE 5 — Customisations : `AGENTS.md`, Instructions, Skills, Agents & MCP
...
**Étapes à réaliser** : Lister les skills disponibles en tapant `/` dans la REPL.
...
## 🎬 EXERCICE 6 — Gestion du Contexte

### 🎯 Objectif
Observer que **ce que voit Copilot** détermine la qualité ET la facture.
...
## 🎬 EXERCICE 7 — Bonnes Pratiques de Prompting
...
## 🎬 EXERCICE 8 — Optimisation des Tokens (Caveman Mode)

### 🎯 Objectif
Observer qu'un **agent custom** peut diviser par 3–5 les tokens de sortie sans perdre l'info utile.
...
À vérifier **avant de démarrer** l'atelier :

- [ ] `node --version` ≥ 22
- [ ] `npm --version` ≥ 10
- [ ] `copilot --version` retourne une version
- [ ] `docker ps` répond (sinon sauter l'exercice MCP awesome-copilot — il est optionnel)
...
- [ ] Tableau de suivi tokens ouvert (papier ou onglet)
- [ ] Branche `lab/categories` prête pour l'EXERCICE 4c
...
git checkout main
git branch -D lab/categories 2>$null   # si créée
```
...
→ Vérifier que vous l'avez bien enregistré via `/mcp add` dans `~/.copilot/mcp-config.json` (voir [Pré-requis](#enregistrer-les-serveurs-mcp-dans-le-cli)), que `npx` est dans le `PATH`, et relancer `copilot`.

**MCP `awesome-copilot` en erreur**
→ Docker n'est pas lancé. Ouvrir Docker Desktop, attendre qu'il soit prêt, relancer `copilot`. (Optionnel pour cet atelier.)

**`npm test` échoue après l'exercice Autopilot**
...
## 🔄 Reproduire l'atelier — Guide express
...
3. Démarrer l'atelier (autre terminal) — activer la télémétrie :
...
```bash
# 4. Dérouler les 8 exercices en notant /usage après chacun
